...
measurement plan. In \Cref{section:further_results} we discuss properties of
...
We present the approach of sampling the $n$th arm in \Cref{alg:TXTS}.
...
\begin{lemma}\label{lemma:infinite_measurement}
  ...
  \end{align}
\end{lemma}
Recalling \Cref{lemma:finite_measurement}, this is particularly interesting as
...
\Cref{fig:measurement_plan} indicates the empirical sampling frequencies. The
...
allocation from \Cref{section:concrete_optimal_allocation}, the uniform
...
\Cref{fig:measurement_plan_comparison}. We hope to convey that TXTS steadily
...
\Cref{fig:algorithm_coefficients}. The individual $C_{j, i}$ were computed as
described in \Cref{section:concrete_optimal_allocation}. Seeing a discrepancy
...
\end{align}
If \Cref{eq:missing_link} is satisfied, the
probability $\psi_{n, i}$ is exponentially small in $n$ and therefore the sum
from \Cref{proposition:optimality_sufficient_condition} finite by the geometric
series.

However, this crucial condition from \Cref{eq:missing_link} is not naturally
...
this is in \Cref{lemma:if_converged_then_converged}.

As we are not convinced that \Cref{eq:missing_link} generally holds true, we
...
\textbf{\Cref{lemma:finite_measurement}}
For $\mathcal{I} = \{l: \sum_{n=1}^\infty \psi_{n, l} < \infty\}$ it holds
    that:
\begin{itemize}
  \item $\forall l \notin \mathcal{I}, \forall \epsilon: \Pi_n(\{\theta:
...
\textbf{\Cref{proposition:measurement_plan_arm}}
\begin{align}
  \psi_{n, l} &\leq \frac{1}{2}((1 - \alpha_{n, l}) \sum_{S: l \in S}
      \frac{\alpha_{n, S}}{1 - \alpha_{n, S}} + \alpha_{n, l} \sum_{S': l
      \notin S'} \frac{\alpha_{n, S'}}{1 - \alpha_{n, S'}}) \\
...
\textbf{\Cref{proposition:measurement_pan_set}}
\begin{align}
  \psi_{n, S} &= \frac{1}{2} \alpha_{n, S} +  \Pr[I_n \in S | S_1 \neq S] (1 - \alpha_{n, S}) \\
  \psi_{n, S} &= \frac{\alpha_{n, S}}{2} +  \frac{\alpha_{n, S}}{2} \sum_{S'\neq S} \frac{\alpha_{n, S'}}{1 - \alpha_{n, S'}} + \Pr[S_1, S_2 \neq S \wedge I_n \in S]
\end{align}
...
\textbf{\Cref{lemma:infinite_measurement}}
  \begin{align}
    \sum_{n \in \mathbb{N}} \psi_{n, S} \rightarrow \infty
  \end{align}
\begin{proof}
...
\textbf{\Cref{lemma:psi_convergence}}
\begin{align}
  \alpha_{n, S} \rightarrow 1 \Rightarrow \psi_{n, S} \rightarrow \frac{1}{2}
\end{align}
\begin{proof}
...
\textbf{\Cref{lemma:measurement_plan_bound_max}}
If $\alpha_{n, S^*} \rightarrow 1$, then for all $i \notin S^*$ and for all
$j \in S^*$:
\begin{align}
  \psi_{n, i} &\leq \frac{\alpha_{n, i}}{\max_{S' \neq S^*} \alpha_{n, S'}} \\
...
\textbf{\Cref{lemma:fraction_bound}}
If $\bar{\psi}_{n, S^*} \rightarrow 1/2$, then there exists a sequence
$\epsilon_n \rightarrow 0$ such that $\forall i \notin S^*, \forall n \in
\mathbb{N}$:
\begin{align}
...
\end{align}
\begin{proof}
  Relying on \Cref{lemma:measurement_plan_bound_max}, we first want to find a
  ...
  \begin{align}
    ...
    &= \exp\{-n( \min_{j \in S^*} C_{j, i}(\bar{\psi}_j, \bar{\psi}_i) -
        \Gamma^{\frac{1}{2}*} - \epsilon_n) \}
  \end{align}
  Plugging this into \Cref{lemma:measurement_plan_bound_max} yields the desired
  result.
\end{proof}

\textbf{\Cref{lemma:if_converged_then_converged}}
If $\forall j \in S^* \bar{\psi}_{n, j} \rightarrow \psi^{\frac{1}{2}*}_j$
then $\forall i \in S^* \bar{\psi}_{n, i} \rightarrow \psi^{\frac{1}{2}*}_i$
and thereby
\begin{align}
...
\begin{proof}
  ...
  \Cref{lemma:C_strictly_increasing} we have shown that $C_{j, i}$ is strictly
  ...
  \end{align}
  By recalling that the latter holds for any $j$, and therefore also the minimizing one and combining it with \Cref{lemma:fraction_bound} we obtain
  \begin{align}
    \psi_{n, i} &\leq \exp\{-n( \min_{j \in S^*} C_{j, i}(\bar{\psi}_j,
        \bar{\psi}_i) - \Gamma^{\frac{1}{2}*} - \epsilon_n) \}\\
      &= \exp\{-n(\delta' - \epsilon_n)\}
  \end{align}
  with $\epsilon_n \rightarrow 0$. Remembering that this holds for all oversampled $i$, we plug this into the condition of \Cref{proposition:optimality_sufficient_condition} yields
  \begin{align}
    \sum_{n \in \mathbb{N}} \psi_{n, i}
      ...
      &< \infty
  \end{align}
  by the geometric series. For the converged $j \in S^*$ the sum is trivially finite. Hence the sufficient condition of optimality from \Cref{proposition:optimality_sufficient_condition} is satisfied.
\end{proof}

\textbf{\Cref{lemma:limsup_undersampling}}
Given $\sum_n(\frac{1}{2} - \psi_{n, l}) \mathbb{I}[\bar{\psi}_{n, l} \leq
  \psi_l^* - \delta] < \infty$ we have that
\[\limsup \bar{\psi}_{n, l} \geq \psi_j^* \text{ and } \liminf \bar{\psi}_{n,
    l} \geq \psi_j^*\]
...
\textbf{\Cref{lemma:psi_undersampled}}
...
\begin{proof}
  ...
  \Cref{lemma:fraction_bound}. The last step leverages the fact that a
