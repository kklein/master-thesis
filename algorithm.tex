...
The following lemma formalizes three seemingly intuitive statements. First,
...
\begin{proof}[\Cref{lemma:finite_measurement}]
  ...
  \epsilon} = \{\theta \in \Theta | \min_{l_1 \in S} \theta_j \geq \max_{l_2
  \notin S} \theta_i + \epsilon\}$. Also, let us define $\rho^* = \max_{S
  \not\subseteq \mathcal{I}} \min_{l_1 \in S} \theta^*_{l_1}$. Intuitively,
  $\rho^*$ represents a metric on the best set of arms under the true values, of
  which every arm is sampled infinitely often under infinite samples. Assuming \bar{\theta}$ to be the largest possible parameter, choose
  $\epsilon$ such that $\rho^* + 2\epsilon < \bar{\theta}$. For $S \subset
  \mathcal{I}$, we have:
  \begin{align}
