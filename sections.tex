...
\begin{proof}[\Cref{lemma:C_strictly_increasing}]
  ...
  We fix two reals $\psi_{j1} < \psi_{j2}$ from $[0, 1]$ as well as their counterparts $x_1 = \argmin_{x \in \mathbb{R}} f(x, \psi_{j1})$ and $x_2 = \argmin_{x \in \mathbb{R}} f(x, \psi_{j2})$. Hence our goal is to show that $f(x_1, \psi_{j1}) < f(x_1, \psi_{j2})$ for minimizing $x_1$. By \Cref{lemma:C_unique_solution} both $x_1$ and $x_2$ are unique. As $x_1$ is unique, we have
  \begin{align}
